...
insert into osm.county(osm_id, name, uppername, geom) 
	SELECT planet_osm_polygon.osm_id, 
    	...
    	st_multi(planet_osm_polygon.way)::geometry(MultiPolygon, 0) as way
   	FROM planet_osm_polygon
  	WHERE (planet_osm_polygon.place = 'county'::text OR planet_osm_polygon.admin_level = '5'::text AND planet_osm_polygon.name = 'Budapest'::text) AND planet_osm_polygon.boundary = 'administrative'::text;
delete from osm.county where not st_intersects(st_centroid(geom), (select geom from osm.country limit 1));


drop table if exists osm.city;
create table osm.city(
	id serial not null primary key,
	osm_id integer,
	name text,
	uppername text,
...
create index gix_city on osm.city using gist(geom);
delete from osm.city;
insert into osm.city(osm_id, name, uppername, geom) 
	SELECT planet_osm_polygon.osm_id, 
    	planet_osm_polygon.name as name,  
    	upper(planet_osm_polygon.name) AS uppername,
    	st_multi(planet_osm_polygon.way)::geometry(MultiPolygon, 0) as way
   	FROM planet_osm_polygon
  	WHERE (planet_osm_polygon.place = 'city'::text OR planet_osm_polygon.admin_level = '7'::text) AND planet_osm_polygon.boundary = 'administrative'::text;
delete from osm.city where not st_intersects(st_centroid(geom), (select geom from osm.country limit 1));
